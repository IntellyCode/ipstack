...
mod tests {
    ...
    fn peer_ack(seq: u32, ack: u32) -> NetworkPacket {
        ...
        create_raw_packet(src, dst, |_, _| usize::MAX, ACK, TTL, seq, ack, u16::MAX, Vec::new(), None).unwrap()
    }
    ...
    async fn a_reset_stops_retransmission() {
        ...
        let (src, dst) = (PEER.parse().unwrap(), LOCAL.parse().unwrap());
        let reset = create_raw_packet(src, dst, |_, _| usize::MAX, RST, TTL, 1001, local_seq, u16::MAX, Vec::new(), None).unwrap();
